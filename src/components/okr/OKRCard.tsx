...
export function OKRCard({ objective, isSelected, onClick }: OKRCardProps) {
  ...
  return (
    <button
      onClick={onClick}
      className={`w-full text-left p-3 lg:p-4 rounded-lg border-2 transition-all touch-manipulation ${
        isSelected
          ? 'border-purple-500 bg-purple-50 shadow-sm'
          : 'border-gray-200 bg-white hover:border-gray-300 hover:shadow-sm active:bg-gray-50'
      }`}
    >
      <div className="flex items-start gap-2.5 lg:gap-3 mb-2.5 lg:mb-3">
        <div className={`p-1.5 lg:p-2 rounded-lg ${isSelected ? 'bg-purple-100' : 'bg-gray-100'}`}>
          <Target className={`w-4 h-4 ${isSelected ? 'text-purple-600' : 'text-gray-600'}`} />
        </div>
        <div className="flex-1 min-w-0">
          <div className="flex items-start justify-between gap-2 mb-1">
            <h3 className="text-sm lg:text-base font-medium text-gray-900 line-clamp-2 flex-1">
              {objective.title}
            </h3>
            {safeProgress >= 100 && (
              <span className="px-2 py-0.5 bg-blue-100 text-blue-700 text-xs font-medium rounded-full whitespace-nowrap flex-shrink-0">
                ✓ Hoàn thành
              </span>
            )}
          </div>
      ...
      <div className="flex items-center justify-between mb-2.5 lg:mb-3">
        <span className={`inline-flex items-center px-2 lg:px-2.5 py-0.5 rounded-full text-xs font-medium ${status.color} text-white`}>
          {status.label}
        </span>
      ...
      <div className="space-y-1.5 lg:space-y-2">
        ...
        <div className="w-full bg-gray-200 rounded-full h-2 lg:h-1.5 overflow-hidden">
          <div
